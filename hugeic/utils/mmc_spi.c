	...
	unsigned long timeout)
{
    ...
    //print_hex_dump(KERN_ERR, "READ DATA: ", 0, 16, 1, 
    //                    host->t.rx_buf, host->t.len, 0);
	if (host->mmc->use_spi_crc) {
...
crc_recover:
	...
	if (status == 0 && mrq->data) {
		 ...
		 */
		if (mrq->data->error == -EILSEQ && crc_retry && mmc->card->type != MMC_TYPE_SDIO) {
